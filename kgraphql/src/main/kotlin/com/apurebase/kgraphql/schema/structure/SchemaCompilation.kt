...
class SchemaCompilation(
    ...
    private suspend fun handleSubscriptions(): Type {
        return Type.OperationObject(
            "Subscription",
            "Subscription object",
            definition.subscriptions.map { handleOperation(it) } + definition.subscriptionsFlow.map { handleOperation(it) })
    }
    ...
    private suspend fun handlePossiblyWrappedType(kType: KType, typeCategory: TypeCategory): Type = try {
        when {
            ...
            kType.jvmErasure == Flow::class -> handlePossiblyWrappedType(kType.arguments.first().type!!, typeCategory)
