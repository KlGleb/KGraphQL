...
data class MutableSchemaDefinition(
    ...
    private val subscriptionFlows: ArrayList<SubscriptionFlowDef<*>> = arrayListOf(),
    ...
    fun addSubscription(subscription: SubscriptionDef<*>) {
        if (subscription.checkEqualName(subscriptions) || subscription.checkEqualName(subscriptionFlows)) {
            throw SchemaException("Cannot add subscription with duplicated name ${subscription.name}")
        }
        subscriptions.add(subscription)
    }

    fun addSubscriptionFlow(subscription: SubscriptionFlowDef<*>) {
        if (subscription.checkEqualName(subscriptions) || subscription.checkEqualName(subscriptionFlows)) {
            throw SchemaException("Cannot add subscription with duplicated name ${subscription.name}")
        }
        subscriptionFlows.add(subscription)
    }
